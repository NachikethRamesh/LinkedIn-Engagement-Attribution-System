from __future__ import annotations

from typing import Any

from app.writeback.adapters.base import BaseWritebackAdapter
from app.writeback.types import DeliveryResult


class WebhookGenericWritebackAdapter(BaseWritebackAdapter):
    target_type = "webhook_generic"

    def deliver(
        self,
        payload: dict[str, Any],
        endpoint_url: str | None,
        timeout_seconds: int = 15,
        **kwargs: Any,
    ) -> DeliveryResult:
        auth_headers = kwargs.get("auth_headers")
        if not endpoint_url:
            return DeliveryResult(
                status="failed",
                response_json={"target_type": self.target_type},
                error_message="No webhook endpoint configured for webhook_generic target",
            )
        return self._post_json_with_headers(
            endpoint_url=endpoint_url,
            payload=payload,
            timeout_seconds=timeout_seconds,
            headers=auth_headers if isinstance(auth_headers, dict) else None,
        )
